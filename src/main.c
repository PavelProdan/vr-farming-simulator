...
// --- New Game Mechanic Enums & Structs ---
typedef enum {
    ITEM_NONE = -1, // Represents an empty inventory slot
    ITEM_FOOD,
    ITEM_EGG,
    ITEM_MILK,
    ITEM_STEAK,
...
Texture2D inventoryItemTextures[ITEM_TYPE_COUNT]; // To store textures like egg.png, food.png
...
PlayerInventorySlot playerInventory = {ITEM_NONE, 0};
...
BarnStorage barnStorage = {200, 0, 0, 0}; // Initial food in barn

float playerCoins = 100.0f; // Starting coins

// Prices
#define PRICE_FOOD_BUY 5.0f
#define PRICE_EGG_SELL 2.0f
#define PRICE_MILK_SELL 8.0f
#define PRICE_STEAK_SELL 15.0f
#define PRICE_CHICKEN_BUY 20.0f
#define PRICE_PIG_BUY 50.0f
#define PRICE_COW_BUY 100.0f

// Animal Stats (per category)
typedef struct {
    float hunger; // 0-100%
    float productCooldownTimer; // Time until next product is ready
    float timeSinceLastFed; // Tracks time for hunger depletion
    bool productReady;
} AnimalCategoryStats;

AnimalCategoryStats animalStats[ANIMAL_COUNT]; // One for each AnimalType
...
#define COW_PRODUCTION_TIME 60.0f   // seconds per milk per cow

// Interaction Menu Enums
typedef enum {
    MENU_NONE,
    MENU_BARN,
    MENU_BANK,
    ...
    MENU_FEED_PIGS,
    MENU_FEED_COWS,
    MENU_COLLECT_CHICKENS,
    MENU_COLLECT_PIGS,
    MENU_COLLECT_COWS
} ActiveMenu;

ActiveMenu currentMenu = MENU_NONE;
int menuSelectedItem = 0; // For navigating menu options

// Flags for showing interaction prompts
bool showBarnPrompt = false;
bool showBankPrompt = false;
bool showChickenInteractionPrompt = false;
bool showPigInteractionPrompt = false;
bool showCowInteractionPrompt = false;

// --- Interaction & Menu Helper Functions ---
bool IsNearBuilding(Vector3 playerPos, Vector3 buildingPos, float interactionRadius) {
    // Use squared distance for better performance - avoid square root
    float distSq = Vector3DistanceSqr(playerPos, buildingPos);
    bool isNear = distSq < (interactionRadius * interactionRadius);
    
    if (isNear) {
        TraceLog(LOG_INFO, "Player is near building at (%.2f, %.2f, %.2f). Distance: %.2f, Radius: %.2f", 
                 buildingPos.x, buildingPos.y, buildingPos.z, sqrtf(distSq), interactionRadius);
    }
    
    return isNear;
}

void AddToInventory(InventoryItemType itemType, int quantity) {
    if (playerInventory.type == ITEM_NONE || playerInventory.type == itemType) {
        playerInventory.type = itemType;
        playerInventory.quantity += quantity;
        TraceLog(LOG_INFO, "Added %d %s to inventory. Total: %d", quantity, inventoryItemNames[itemType], playerInventory.quantity);
    } else {
        // Inventory has a different item type
        TraceLog(LOG_WARNING, "Cannot add %s, inventory contains %s. Clear inventory first.", inventoryItemNames[itemType], inventoryItemNames[playerInventory.type]);
        // Optionally, show a message to the player here
    }
}

void RemoveFromInventory(InventoryItemType itemType, int quantity) {
    if (playerInventory.type == itemType && playerInventory.quantity >= quantity) {
        playerInventory.quantity -= quantity;
        TraceLog(LOG_INFO, "Removed %d %s from inventory. Remaining: %d", quantity, inventoryItemNames[itemType], playerInventory.quantity);
        if (playerInventory.quantity == 0) {
            playerInventory.type = ITEM_NONE; // Clear type if quantity is zero
            TraceLog(LOG_INFO, "Inventory slot for %s is now empty.", inventoryItemNames[itemType]);
        }
    } else {
        TraceLog(LOG_WARNING, "Cannot remove %d %s from inventory. Item not found or insufficient quantity.", quantity, inventoryItemNames[itemType]);
    }
}

void ClearInventory() {
    if (playerInventory.type != ITEM_NONE) {
        TraceLog(LOG_INFO, "Clearing inventory of %d %s.", playerInventory.quantity, inventoryItemNames[playerInventory.type]);
    }
    playerInventory.type = ITEM_NONE;
    playerInventory.quantity = 0;
}

// --- End Interaction & Menu Helper Functions ---


// --- End New Game Mechanic Enums & Structs ---
...
void UpdateCameraCustom(Camera *camera, int mode); // Forward declaration
...
void GenerateSmoothRoad(CustomRoad* road, float rWidth, Texture2D rTexture) {
    ...
    TraceLog(LOG_INFO, "Smooth road created successfully for '%s' with %d points", 
             road->name, road->numPoints);
}
...
void DrawHumanStartMenu(Human *h) {
    // Only draw if the human is active and in the specific state for the start menu
    if (h->active && h->state == HUMAN_STATE_IDLE_AT_INTERSECTION) {
        TraceLog(LOG_INFO, "DrawHumanStartMenu: Menu IS ACTIVE and being drawn. human.active = %d, human.state = %d", h->active, h->state);
...
int main(void)
{
    ...
    // Load inventory item textures
    inventoryItemTextures[ITEM_FOOD] = LoadTexture("inventory/food.png");
    inventoryItemTextures[ITEM_EGG] = LoadTexture("inventory/egg.png");
    inventoryItemTextures[ITEM_MILK] = LoadTexture("inventory/milk.png");
    inventoryItemTextures[ITEM_STEAK] = LoadTexture("inventory/steak.png");
    
    // Log loading of textures
    TraceLog(LOG_INFO, "Loading inventory textures:");
    TraceLog(LOG_INFO, "- Food texture ID: %d", inventoryItemTextures[ITEM_FOOD].id);
    TraceLog(LOG_INFO, "- Egg texture ID: %d", inventoryItemTextures[ITEM_EGG].id);
    TraceLog(LOG_INFO, "- Milk texture ID: %d", inventoryItemTextures[ITEM_MILK].id);
    TraceLog(LOG_INFO, "- Steak texture ID: %d", inventoryItemTextures[ITEM_STEAK].id);
    
    // Check if textures loaded successfully
    for (int i = 0; i < ITEM_TYPE_COUNT; i++) {
        if (inventoryItemTextures[i].id == 0) {
            TraceLog(LOG_WARNING, "Failed to load texture for item type %s", inventoryItemNames[i]);
        }
    }
    
    // Initialize animal stats
    for (int i = 0; i < ANIMAL_COUNT; i++) {
        animalStats[i].hunger = 100.0f;
        animalStats[i].productCooldownTimer = 0.0f;
        animalStats[i].timeSinceLastFed = 0.0f;
        animalStats[i].productReady = false;
    }
    ...
    while (!WindowShouldClose())
    {
        ...
        // Log human state at the start of the loop
        TraceLog(LOG_INFO, "Loop Start: human.active = %d, human.state = %d, currentMenu = %d", human.active, human.state, currentMenu);
        ...
        bool interactionKeyPressed = IsKeyPressed(KEY_F); // Use F for interaction

        // --- Proximity Detection and Menu Activation ---
        // Disable other menus if human start menu is active
        if (!(human.active && human.state == HUMAN_STATE_IDLE_AT_INTERSECTION)) {
            TraceLog(LOG_INFO, "Interaction logic check: human.active = %d, human.state = %d. Interaction logic WILL RUN.", human.active, human.state);

            // Reset prompt flags each frame this logic runs
            showBarnPrompt = false;
            showBankPrompt = false;
            showChickenInteractionPrompt = false;
            showPigInteractionPrompt = false;
            showCowInteractionPrompt = false;

            bool nearBarn = IsNearBuilding(playerPos, buildings[0].position, 10.0f); // Barn is buildings[0]
            bool nearBank = IsNearBuilding(playerPos, buildings[2].position, 10.0f); // Bank is buildings[2]

            if (currentMenu == MENU_NONE) {
                if (nearBarn) {
                    showBarnPrompt = true;
                    if (interactionKeyPressed) {
                        currentMenu = MENU_BARN;
                        menuSelectedItem = 0;
                        TraceLog(LOG_INFO, "Player pressed F near barn - opening barn menu");
                    }
                } else if (nearBank) {
                    showBankPrompt = true;
                    if (interactionKeyPressed) {
                        currentMenu = MENU_BANK;
                        menuSelectedItem = 0;
                        TraceLog(LOG_INFO, "Player pressed F near bank - opening bank menu");
                    }
                } else {
                    // Check for animal interaction (feeding/collection) only if no building menu is prompted
                    // Feeding Chickens
                    if (animalCountByType[ANIMAL_CHICKEN] > 0 && IsNearBuilding(playerPos, ENCLOSURE_CENTER_2, 15.0f)) { // Chicken enclosure - larger radius
                        showChickenInteractionPrompt = true;
                        TraceLog(LOG_INFO, "Player is near chicken enclosure");
                        
                        if (interactionKeyPressed) {
                            // Show the appropriate menu based on player's inventory and animal status
                            if (playerInventory.type == ITEM_FOOD && playerInventory.quantity > 0 && animalStats[ANIMAL_CHICKEN].hunger < 100.0f) {
                                currentMenu = MENU_FEED_CHICKENS;
                                TraceLog(LOG_INFO, "Opening chicken feeding menu");
                            } else if (playerInventory.type == ITEM_NONE && animalStats[ANIMAL_CHICKEN].productReady) {
                                currentMenu = MENU_COLLECT_CHICKENS;
                                TraceLog(LOG_INFO, "Opening chicken collecting menu");
                            }
                            menuSelectedItem = 0;
                        }
                    }
                    // Feeding Pigs
                    else if (animalCountByType[ANIMAL_PIG] > 0 && IsNearBuilding(playerPos, ENCLOSURE_CENTER_1, 15.0f)) { // Pig enclosure - larger radius
                        showPigInteractionPrompt = true;
                        TraceLog(LOG_INFO, "Player is near pig enclosure");
                        
                        if (interactionKeyPressed) {
                            // Show the appropriate menu based on player's inventory and animal status
                            if (playerInventory.type == ITEM_FOOD && playerInventory.quantity > 0 && animalStats[ANIMAL_PIG].hunger < 100.0f) {
                                currentMenu = MENU_FEED_PIGS;
                                TraceLog(LOG_INFO, "Opening pig feeding menu");
                            } else if (playerInventory.type == ITEM_NONE && animalStats[ANIMAL_PIG].productReady) {
                                currentMenu = MENU_COLLECT_PIGS;
                                TraceLog(LOG_INFO, "Opening pig collecting menu");
                            }
                            menuSelectedItem = 0;
                        }
                    }
                    // Feeding/Collecting Cows (near any cow)
                    else if (animalCountByType[ANIMAL_COW] > 0) {
                        bool nearAnyCow = false;
                        Vector3 nearCowPosition;
                        for (int i = 0; i < animalCount; i++) {
                            if (animals[i].active && animals[i].type == ANIMAL_COW) {
                                if (IsNearBuilding(playerPos, animals[i].position, 10.0f)) { // Larger radius
                                    nearAnyCow = true;
                                    nearCowPosition = animals[i].position;
                                    break;
                                }
                            }
                        }
                        if (nearAnyCow) {
                            showCowInteractionPrompt = true;
                            TraceLog(LOG_INFO, "Player is near cow");
                            
                            if (interactionKeyPressed) {
                                // Show the appropriate menu based on player's inventory and animal status
                                if (playerInventory.type == ITEM_FOOD && playerInventory.quantity > 0 && animalStats[ANIMAL_COW].hunger < 100.0f) {
                                    currentMenu = MENU_FEED_COWS;
                                    TraceLog(LOG_INFO, "Opening cow feeding menu");
                                } else if (playerInventory.type == ITEM_NONE && animalStats[ANIMAL_COW].productReady) {
                                    currentMenu = MENU_COLLECT_COWS;
                                    TraceLog(LOG_INFO, "Opening cow collecting menu");
                                }
                                menuSelectedItem = 0;
                            }
                        }
            ...
            } else if (interactionKeyPressed && (currentMenu == MENU_BARN || currentMenu == MENU_BANK || currentMenu == MENU_FEED_CHICKENS || currentMenu == MENU_FEED_PIGS || currentMenu == MENU_FEED_COWS || currentMenu == MENU_COLLECT_CHICKENS || currentMenu == MENU_COLLECT_PIGS || currentMenu == MENU_COLLECT_COWS)) { 
                 currentMenu = MENU_NONE; // Allow F to close any open menu if it was used to open/confirm
                 TraceLog(LOG_INFO, "Interaction menu closed by F key.");
            }
        }

        // Update Animal Hunger and Production (using AnimalCategoryStats)
        for (int type = 0; type < ANIMAL_COUNT; type++) {
            if (animalCountByType[type] == 0) continue; // Skip if no animals of this type

            animalStats[type].timeSinceLastFed += deltaTime;

            // Hunger depletion
            if (animalStats[type].timeSinceLastFed >= HUNGER_DEPLETION_INTERVAL) {
                animalStats[type].timeSinceLastFed = 0.0f; // Reset interval timer
                float depletionRate = 0.0f;
                switch(type) {
                    case ANIMAL_CHICKEN: depletionRate = CHICKEN_HUNGER_DEPLETION_RATE; break;
                    case ANIMAL_PIG:     depletionRate = PIG_HUNGER_DEPLETION_RATE; break;
                    case ANIMAL_COW:     depletionRate = COW_HUNGER_DEPLETION_RATE; break;
                }
                animalStats[type].hunger -= depletionRate;
                if (animalStats[type].hunger < 0) animalStats[type].hunger = 0;
            }

            // Production
            if (animalStats[type].hunger > 20.0f) { // Animals only produce if hunger > 20%
                 if (!animalStats[type].productReady) { // Only countdown if product is not already ready
                    animalStats[type].productCooldownTimer -= deltaTime;
                    if (animalStats[type].productCooldownTimer <= 0) {
                        animalStats[type].productReady = true;
                        // Cooldown will be reset when product is collected
                        // Set a minimal timer to avoid immediate re-trigger if collection fails
                        animalStats[type].productCooldownTimer = 0.1f; 
                    }
                }
            } else {
                // If hunger is too low, pause production timer but don't reset it completely.
                // It will resume when animals are fed again. Product can still be ready if it became ready before hunger dropped.
            }
        }
        ...
        // TEST: Draw a simple red square at top-left to see if any 2D drawing works after start menu closes
        if (!(human.active && human.state == HUMAN_STATE_IDLE_AT_INTERSECTION)) {
            // DrawRectangle(0, 0, 100, 100, RED); // Test red square
            TraceLog(LOG_INFO, "TEST: Drawing RED square because human start menu is NOT active.");
        } else {
            TraceLog(LOG_INFO, "TEST: NOT drawing RED square because human start menu IS active.");
        }
        ...
        DrawRectangleLines(invDisplayX, invDisplayY, invDisplayWidth, invDisplayHeight, LIGHTGRAY);

        // Title for the inventory
        const char* invTitle = "Player Inventory";
        int invTitleFontSize = 24;
        DrawText(invTitle, invDisplayX + invDisplayWidth/2 - MeasureText(invTitle, invTitleFontSize)/2, invDisplayY + 10, invTitleFontSize, WHITE);

        if (playerInventory.type != ITEM_NONE && playerInventory.quantity > 0) {
            Texture2D itemTex = inventoryItemTextures[playerInventory.type];
            if (itemTex.id > 0) {
                // Display item image (200x200)
                float imageSize = 200.0f;
                float imageX = invDisplayX + (invDisplayWidth - imageSize) / 2; // Center image horizontally
                float imageY = invDisplayY + invTitleFontSize + 20; // Position image below title

                DrawTexturePro(itemTex,
                               (Rectangle){0, 0, (float)itemTex.width, (float)itemTex.height}, // Source rectangle
                               (Rectangle){imageX, imageY, imageSize, imageSize}, // Destination rectangle (200x200)
                               (Vector2){0,0}, // Origin
                               0.0f, // Rotation
                               WHITE);

                // Display item name and quantity below the image
                int textFontSize = 22;
                const char* itemText = TextFormat("%s: %d", inventoryItemNames[playerInventory.type], playerInventory.quantity);
                int textWidth = MeasureText(itemText, textFontSize);
                DrawText(itemText,
                         invDisplayX + (invDisplayWidth - textWidth) / 2, // Center text horizontally
                         imageY + imageSize + 10, // Position text below image with padding
                         textFontSize, WHITE);
            } else {
                // Fallback if texture is not loaded, just draw text (centered)
                const char* fallbackText = TextFormat("%s: %d (Icon Missing)", inventoryItemNames[playerInventory.type], playerInventory.quantity);
                int fallbackFontSize = 20;
                int fallbackTextWidth = MeasureText(fallbackText, fallbackFontSize);
                DrawText(fallbackText,
                         invDisplayX + (invDisplayWidth - fallbackTextWidth)/2,
                         invDisplayY + invDisplayHeight/2 - fallbackFontSize/2, // Centered in the box
                         fallbackFontSize, YELLOW);
            }
        } else {
            const char* emptyText = "Inventory Empty";
            int emptyFontSize = 22;
            DrawText(emptyText, 
                     invDisplayX + invDisplayWidth/2 - MeasureText(emptyText, emptyFontSize)/2, 
                     invDisplayY + invDisplayHeight/2 - emptyFontSize/2, // Centered in the box
                     emptyFontSize, LIGHTGRAY);
        }

        // --- Draw Animal HUD (Top Right) ---
        int hudBoxWidth = 250;
        int hudBoxHeight = 120;
        int hudBoxX = screenWidth - hudBoxWidth - 10;
        int hudBoxY = 10;

        TraceLog(LOG_INFO, "Drawing Animal HUD: X=%d, Y=%d, W=%d, H=%d", hudBoxX, hudBoxY, hudBoxWidth, hudBoxHeight);

        DrawRectangle(hudBoxX, hudBoxY, hudBoxWidth, hudBoxHeight, Fade(DARKBLUE, 0.7f));
        DrawRectangleLines(hudBoxX, hudBoxY, hudBoxWidth, hudBoxHeight, SKYBLUE);
        DrawText("Animal Status", hudBoxX + hudBoxWidth/2 - MeasureText("Animal Status", 20)/2, hudBoxY + 5, 20, WHITE);

        int hudTextY = hudBoxY + 30;
        if (animalCountByType[ANIMAL_CHICKEN] > 0) {
            DrawText(TextFormat("Chickens (%d): Hunger %.0f%%", animalCountByType[ANIMAL_CHICKEN], animalStats[ANIMAL_CHICKEN].hunger), hudBoxX + 10, hudTextY, 10, animalStats[ANIMAL_CHICKEN].hunger < 30 ? RED : WHITE);
            if (animalStats[ANIMAL_CHICKEN].productReady) DrawText("Eggs Ready!", hudBoxX + 150, hudTextY, 10, GREEN);
            hudTextY += 15;
        }
        if (animalCountByType[ANIMAL_PIG] > 0) {
            DrawText(TextFormat("Pigs (%d): Hunger %.0f%%", animalCountByType[ANIMAL_PIG], animalStats[ANIMAL_PIG].hunger), hudBoxX + 10, hudTextY, 10, animalStats[ANIMAL_PIG].hunger < 30 ? RED : WHITE);
            if (animalStats[ANIMAL_PIG].productReady) DrawText("Steak Ready!", hudBoxX + 150, hudTextY, 10, GREEN);
            hudTextY += 15;
        }
        if (animalCountByType[ANIMAL_COW] > 0) {
            DrawText(TextFormat("Cows (%d): Hunger %.0f%%", animalCountByType[ANIMAL_COW], animalStats[ANIMAL_COW].hunger), hudBoxX + 10, hudTextY, 10, animalStats[ANIMAL_COW].hunger < 30 ? RED : WHITE);
            if (animalStats[ANIMAL_COW].productReady) DrawText("Milk Ready!", hudBoxX + 150, hudTextY, 10, GREEN);
            hudTextY += 15;
        }
        if (hudTextY == hudBoxY + 30) { // No animals yet
             DrawText("No animals on the farm yet.", hudBoxX + 10, hudTextY, 10, LIGHTGRAY);
        }
        
        // --- Draw Barn Storage info (Top Left) ---
        // DrawRectangle(5, 235, 220, 130, Fade(BROWN, 0.7f));
        // DrawRectangleLines(5, 235, 220, 130, DARKBROWN);
        // DrawText("Barn Storage", 15, 245, 20, WHITE);
        // DrawText(TextFormat("Food: %d", barnStorage.food), 15, 275, 15, WHITE);
        // DrawText(TextFormat("Eggs: %d", barnStorage.eggs), 15, 295, 15, WHITE);
        // DrawText(TextFormat("Milk: %d", barnStorage.milk), 15, 315, 15, WHITE);
        // DrawText(TextFormat("Steak: %d", barnStorage.steak), 15, 335, 15, WHITE);
        
        // --- Draw Player Coins (Next to Animal HUD) ---
        int coinsBoxX = hudBoxX - 120; // Position to the left of Animal HUD
        int coinsBoxY = 10;
        int coinsBoxWidth = 110;
        int coinsBoxHeight = 40;

        TraceLog(LOG_INFO, "Drawing Coins: X=%d, Y=%d, W=%d, H=%d", coinsBoxX, coinsBoxY, coinsBoxWidth, coinsBoxHeight);

        DrawRectangle(coinsBoxX, coinsBoxY, coinsBoxWidth, coinsBoxHeight, Fade(GOLD, 0.7f));
        DrawRectangleLines(coinsBoxX, coinsBoxY, coinsBoxWidth, coinsBoxHeight, YELLOW);
        DrawText(TextFormat("Coins: %.0f", playerCoins), coinsBoxX + 10, coinsBoxY + 10, 20, BLACK);

        // --- Draw Interaction Prompts --- (New Section)
        if (currentMenu == MENU_NONE && !(human.active && human.state == HUMAN_STATE_IDLE_AT_INTERSECTION)) {
            int promptY = screenHeight - 40; // Position prompts lower, above inventory
            int promptFontSize = 20;
            if (showBarnPrompt) {
                DrawText("[F] Interact with Barn", screenWidth / 2 - MeasureText("[F] Interact with Barn", promptFontSize) / 2, promptY, promptFontSize, WHITE);
            } else if (showBankPrompt) {
                DrawText("[F] Interact with Bank", screenWidth / 2 - MeasureText("[F] Interact with Bank", promptFontSize) / 2, promptY, promptFontSize, WHITE);
            } else if (showChickenInteractionPrompt) {
                DrawText("[F] Interact with Chickens", screenWidth / 2 - MeasureText("[F] Interact with Chickens", promptFontSize) / 2, promptY, promptFontSize, WHITE);
            } else if (showPigInteractionPrompt) {
                DrawText("[F] Interact with Pigs", screenWidth / 2 - MeasureText("[F] Interact with Pigs", promptFontSize) / 2, promptY, promptFontSize, WHITE);
            } else if (showCowInteractionPrompt) {
                DrawText("[F] Interact with Cows", screenWidth / 2 - MeasureText("[F] Interact with Cows", promptFontSize) / 2, promptY, promptFontSize, WHITE);
            }
        }

        // --- Draw Interaction Menus (Barn/Bank) ---
        if (currentMenu == MENU_BARN) {
            // First clear the screen and draw a full screen black background
            ClearBackground(BLACK);
            
            // Draw title in the center top portion
            int titleFontSize = 50;
            DrawText("BARN OPERATIONS", screenWidth/2 - MeasureText("BARN OPERATIONS", titleFontSize)/2, 80, titleFontSize, WHITE);
            
            // Draw an underline below the title
            DrawRectangle(screenWidth/2 - 200, 140, 400, 5, WHITE);

            // Define layout parameters, shifting the barn menu clearly to the right
            int startX = (screenWidth * 2) / 3; // Position menu starting at ~67% of screen width
            int startY = 170;
            int rowSpacing = 40;
            int fontSize = 25;
            int optionFontSize = 30;
            
            // Draw barn storage info in the center top section
            DrawText("BARN STORAGE", startX, startY, fontSize, GOLD);
            startY += rowSpacing;
            
            // Draw barn storage items in single left-aligned column
            DrawText(TextFormat("Food: %d", barnStorage.food), startX, startY, fontSize, WHITE);
            startY += rowSpacing;
            DrawText(TextFormat("Eggs: %d", barnStorage.eggs), startX, startY, fontSize, WHITE);
            startY += rowSpacing;
            DrawText(TextFormat("Milk: %d", barnStorage.milk), startX, startY, fontSize, WHITE);
            startY += rowSpacing;
            DrawText(TextFormat("Steak: %d", barnStorage.steak), startX, startY, fontSize, WHITE);
            // Player inventory section
            startY += rowSpacing + 20;
            DrawText("YOUR INVENTORY", startX, startY, fontSize, GOLD);
            startY += rowSpacing;
            
            if (playerInventory.type != ITEM_NONE && playerInventory.quantity > 0) {
                DrawText(TextFormat("%s: %d", inventoryItemNames[playerInventory.type], playerInventory.quantity), 
                         startX, 
                         startY, fontSize, WHITE);
            } else {
                DrawText("Empty", startX, startY, fontSize, WHITE);
            }

            // Menu options section
            startY += rowSpacing + 20;
            // Define barn menu options and count
            const char* barnOptions[] = {
                "Store Food",
                "Take Food",
                "Store Eggs",
                "Store Milk",
            ...
            int numBarnOptions = sizeof(barnOptions)/sizeof(barnOptions[0]);
            DrawText("MENU OPTIONS", startX, startY, fontSize, GOLD);
            startY += rowSpacing;
            
            // Draw options in a vertical list with proper spacing
            int optionStartY = startY;
            int optionSpacing = 45;
            int padding = 15;
            
            // Draw each option centered
            for (int i = 0; i < numBarnOptions; i++) {
                // Highlight selected option
                if (i == menuSelectedItem) {
                    DrawRectangle(
                        startX - padding,
                        optionStartY + i * optionSpacing - padding,
                        MeasureText(barnOptions[i], optionFontSize) + (padding * 2),
                        optionFontSize + (padding * 2),
                        Fade(DARKBLUE, 0.7f)
                    );
                }
                Color textColor = (i == menuSelectedItem) ? YELLOW : WHITE;
                DrawText(barnOptions[i], 
                         startX, 
                         optionStartY + i * optionSpacing, 
                         optionFontSize, textColor);
            }

            if (IsKeyPressed(KEY_UP)) menuSelectedItem = (menuSelectedItem - 1 + numBarnOptions) % numBarnOptions;
            if (IsKeyPressed(KEY_DOWN)) menuSelectedItem = (menuSelectedItem + 1) % numBarnOptions;
            if (IsKeyPressed(KEY_ENTER)) { // F can also confirm in menu
                switch (menuSelectedItem) {
                    case 0: // Store Food
                        if (playerInventory.type == ITEM_FOOD && playerInventory.quantity > 0) {
                            barnStorage.food += playerInventory.quantity;
                            TraceLog(LOG_INFO, "Stored %d food in barn. Barn total: %d", playerInventory.quantity, barnStorage.food);
                            ClearInventory();
                        } else {
                             TraceLog(LOG_WARNING, "No food in inventory to store.");
                        }
                        break;
                    case 1: // Take Food
                        if (playerInventory.type == ITEM_NONE || playerInventory.type == ITEM_FOOD) {
                            if (barnStorage.food > 0) {
                                int amountToTake = (barnStorage.food >= 50) ? 50 : barnStorage.food;
                                AddToInventory(ITEM_FOOD, amountToTake);
                                barnStorage.food -= amountToTake;
                                TraceLog(LOG_INFO, "Took %d food from barn. Barn remaining: %d", amountToTake, barnStorage.food);
                            } else {
                                TraceLog(LOG_WARNING, "No food in barn to take.");
                            }
                        } else {
                            TraceLog(LOG_WARNING, "Inventory not empty or already contains a different item. Cannot take food.");
                        }
                        break;
                    case 2: // Store Eggs
                        if (playerInventory.type == ITEM_EGG && playerInventory.quantity > 0) {
                            barnStorage.eggs += playerInventory.quantity;
                            TraceLog(LOG_INFO, "Stored %d eggs in barn. Barn total: %d", playerInventory.quantity, barnStorage.eggs);
                            ClearInventory();
                        } else {
                            TraceLog(LOG_WARNING, "No eggs in inventory to store.");
                        }
                        break;
                    case 3: // Store Milk
                        if (playerInventory.type == ITEM_MILK && playerInventory.quantity > 0) {
                            barnStorage.milk += playerInventory.quantity;
                            TraceLog(LOG_INFO, "Stored %d milk in barn. Barn total: %d", playerInventory.quantity, barnStorage.milk);
                            ClearInventory();
                        } else {
                            TraceLog(LOG_WARNING, "No milk in inventory to store.");
                        }
                        break;
                    case 4: // Store Steak
                        if (playerInventory.type == ITEM_STEAK && playerInventory.quantity > 0) {
                            barnStorage.steak += playerInventory.quantity;
                            TraceLog(LOG_INFO, "Stored %d steak in barn. Barn total: %d", playerInventory.quantity, barnStorage.steak);
                            ClearInventory();
                        } else {
                            TraceLog(LOG_WARNING, "No steak in inventory to store.");
                        }
                        break;
                    case 5: // Exit
            ...
            // Clear the screen completely for the bank menu
            ClearBackground(BLACK);
            
            // Draw title in the center top portion
            int titleFontSize = 60;
            DrawText("BANK OPERATIONS", screenWidth/2 - MeasureText("BANK OPERATIONS", titleFontSize)/2, 120, titleFontSize, WHITE);
            ...
            // Display current prices in right column
            int pricesFontSize = 24;
            int pricesX = screenWidth * 2/3;
            int pricesY = 200;
            
            DrawText("Current Prices:", pricesX, pricesY, 36, GOLD);
            pricesY += 50;
            DrawText(TextFormat("Eggs: %.0f coins each", PRICE_EGG_SELL), pricesX, pricesY, pricesFontSize, GOLD);
            pricesY += 40;
            DrawText(TextFormat("Milk: %.0f coins each", PRICE_MILK_SELL), pricesX, pricesY, pricesFontSize, GOLD);
            pricesY += 40;
            DrawText(TextFormat("Steak: %.0f coins each", PRICE_STEAK_SELL), pricesX, pricesY, pricesFontSize, GOLD);
            pricesY += 40;
            DrawText(TextFormat("Food: %.0f coins each", PRICE_FOOD_BUY), pricesX, pricesY, pricesFontSize, GOLD);
            pricesY += 40;
            DrawText(TextFormat("Chicken: %.0f coins", PRICE_CHICKEN_BUY), pricesX, pricesY, pricesFontSize, GOLD);
            pricesY += 40;
            ...
            for (int i = 0; i < numBankOptions; i++) {
                Color textColor = (i == menuSelectedItem) ? YELLOW : WHITE;
                DrawText(bankOptions[i], optionsX, optionsStartY + i * optionSpacing, optionFontSize, textColor);
            }

            if (IsKeyPressed(KEY_UP)) menuSelectedItem = (menuSelectedItem - 1 + numBankOptions) % numBankOptions;
            if (IsKeyPressed(KEY_DOWN)) menuSelectedItem = (menuSelectedItem + 1) % numBankOptions;
            if (IsKeyPressed(KEY_ENTER)) { // ENTER to confirm in menu
                TraceLog(LOG_INFO, "Bank menu option selected: %d", menuSelectedItem);
                
                switch (menuSelectedItem) {
                    case 0: // Sell Eggs
                        if (barnStorage.eggs > 0) {
                            playerCoins += barnStorage.eggs * PRICE_EGG_SELL;
                            TraceLog(LOG_INFO, "Sold %d eggs. Player coins: %.0f", barnStorage.eggs, playerCoins);
                            barnStorage.eggs = 0;
                        } else {
                            TraceLog(LOG_WARNING, "No eggs in barn to sell.");
                        }
                        break;
                    case 1: // Sell Milk
                        if (barnStorage.milk > 0) {
                            playerCoins += barnStorage.milk * PRICE_MILK_SELL;
                            TraceLog(LOG_INFO, "Sold %d milk. Player coins: %.0f", barnStorage.milk, playerCoins);
                            barnStorage.milk = 0;
                        } else {
                            TraceLog(LOG_WARNING, "No milk in barn to sell.");
                        }
                        break;
                    case 2: // Sell Steak
                        if (barnStorage.steak > 0) {
                            playerCoins += barnStorage.steak * PRICE_STEAK_SELL;
                            TraceLog(LOG_INFO, "Sold %d steak. Player coins: %.0f", barnStorage.steak, playerCoins);
                            barnStorage.steak = 0;
                        } else {
                            TraceLog(LOG_WARNING, "No steak in barn to sell.");
                        }
                        break;
                    case 3: // Buy Food
                        if (playerCoins >= PRICE_FOOD_BUY * 50) {
                            playerCoins -= PRICE_FOOD_BUY * 50;
                            barnStorage.food += 50; // Add to barn, player can take it later
                            TraceLog(LOG_INFO, "Bought 50 food. Player coins: %.0f. Barn food: %d", playerCoins, barnStorage.food);
                        } else {
                            TraceLog(LOG_WARNING, "Not enough coins to buy food.");
                        }
                        break;
                    case 4: // Buy Chicken
        ...
        // --- Draw Animal Interaction Menus (Feeding/Collecting) ---
        if (currentMenu == MENU_FEED_CHICKENS || currentMenu == MENU_FEED_PIGS || currentMenu == MENU_FEED_COWS) {
            // Draw fullscreen menu - Replace fade with clear background
            ClearBackground(BLACK);
            
            AnimalType typeToFeed = (currentMenu == MENU_FEED_CHICKENS) ? ANIMAL_CHICKEN : 
            ...
            const char* options[] = {feedOption, "Cancel"};
            int numOptions = 2;

            // Draw options centered
            int optionFontSize = 40; // Increased font size
            int optionSpacing = 70; // Increased spacing
            int optionsStartY = screenHeight/2;
            int padding = 20;
            ...
            if (IsKeyPressed(KEY_UP)) menuSelectedItem = (menuSelectedItem - 1 + numOptions) % numOptions;
            if (IsKeyPressed(KEY_DOWN)) menuSelectedItem = (menuSelectedItem + 1) % numOptions;
            if (IsKeyPressed(KEY_ENTER)) {
                if (menuSelectedItem == 0) { // Feed
                    int foodNeeded = animalCountByType[typeToFeed];
                    if (playerInventory.type == ITEM_FOOD && playerInventory.quantity >= foodNeeded) {
                        RemoveFromInventory(ITEM_FOOD, foodNeeded);
                        animalStats[typeToFeed].hunger = 100.0f;
                        animalStats[typeToFeed].timeSinceLastFed = 0.0f; // Reset depletion timer
                        TraceLog(LOG_INFO, "Fed %s. Consumed %d food.", animalName, foodNeeded);
                    } else {
                        TraceLog(LOG_WARNING, "Not enough food in inventory to feed %s.", animalName);
                    }
                }
                currentMenu = MENU_NONE; // Close menu after action or cancel
            }
            if (IsKeyPressed(KEY_ESCAPE)) currentMenu = MENU_NONE;
        }

        if (currentMenu == MENU_COLLECT_CHICKENS || currentMenu == MENU_COLLECT_PIGS || currentMenu == MENU_COLLECT_COWS) {
            // Draw fullscreen menu - Replace fade with clear background
            ClearBackground(BLACK);

            AnimalType typeToCollect = (currentMenu == MENU_COLLECT_CHICKENS) ? ANIMAL_CHICKEN :
                                       (currentMenu == MENU_COLLECT_PIGS) ? ANIMAL_PIG : ANIMAL_COW;
            InventoryItemType itemToCollect = (typeToCollect == ANIMAL_CHICKEN) ? ITEM_EGG :
                                              (typeToCollect == ANIMAL_PIG) ? ITEM_STEAK : ITEM_MILK;
            const char* productName = inventoryItemNames[itemToCollect];
            ...
            } else if (typeToCollect == ANIMAL_PIG) {
                numProducts = animalCountByType[ANIMAL_PIG]; // 1 steak per pig
            } else if (typeToCollect == ANIMAL_COW) {
                numProducts = animalCountByType[ANIMAL_COW]; // 1 milk per cow
            }
            ...
            const char* options[] = {collectOption, "Cancel"};
            int numOptions = 2;
            ...
            if (IsKeyPressed(KEY_DOWN)) menuSelectedItem = (menuSelectedItem + 1) % numOptions;
            if (IsKeyPressed(KEY_ENTER)) {
                if (menuSelectedItem == 0) { // Collect
                    if (playerInventory.type == ITEM_NONE && animalStats[typeToCollect].productReady && numProducts > 0) {
                        AddToInventory(itemToCollect, numProducts);
                        animalStats[typeToCollect].productReady = false;
                        // Reset cooldown based on type
                        switch(typeToCollect) {
                            case ANIMAL_CHICKEN: animalStats[typeToCollect].productCooldownTimer = CHICKEN_PRODUCTION_TIME; break;
                            case ANIMAL_PIG:     animalStats[typeToCollect].productCooldownTimer = PIG_PRODUCTION_TIME; break;
                            case ANIMAL_COW:     animalStats[typeToCollect].productCooldownTimer = COW_PRODUCTION_TIME; break;
                            default: 
                                // Handle other animal types or provide a default behavior
                                TraceLog(LOG_WARNING, "Product collection cooldown not set for animal type: %d", typeToCollect);
                                break;
                        }
                        TraceLog(LOG_INFO, "Collected %d %s.", numProducts, productName);
                    } else {
                        TraceLog(LOG_WARNING, "Cannot collect. Inventory not empty, products not ready, or no animals.");
                    }
                }
                currentMenu = MENU_NONE; // Close menu
            }
            if (IsKeyPressed(KEY_ESCAPE)) currentMenu = MENU_NONE;
        }
    ...
    // Unload inventory item textures
    for (int i = 0; i < ITEM_TYPE_COUNT; i++) {
        if (inventoryItemTextures[i].id > 0) {
            UnloadTexture(inventoryItemTextures[i]);
        }
    }
...
// Restoring UpdateCameraCustom function definition
void UpdateCameraCustom(Camera *camera, int mode)
{
    ...
    if (mode == CAMERA_FIRST_PERSON || mode == CAMERA_THIRD_PERSON)
    {
        ...
        // Vector3 oldPosition = camera->position; // Not strictly needed if we only apply valid moves
...
bool IsCollisionWithBuilding(Vector3 position, float radius, int* buildingIndex) {
    ...
    for (int i = 0; i < MAX_BUILDINGS; i++) {
        ...
        // Note: Skipping specific buildings like the chicken coop for animal collision 
        // needs a robust identification method (e.g., a type field in Building struct or a known index).
        // The previous attempt using scale was not reliable.
        // For now, all buildings will be checked.
        ...
        if (i == 0) { // barn.glb (index 0)
            ...
            if (buildings[i].scale == FENCE_MODEL_SCALE_CONST) { // Likely a fence
                ...
                // Generic scaling for other unknown buildings.
                buildingRadius = buildings[i].scale * 20.0f; // This factor might need tuning
                if (buildingRadius < 1.5f) buildingRadius = 1.5f; 
            }
        }
